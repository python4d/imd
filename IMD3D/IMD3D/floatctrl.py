...
# -*- coding: iso-8859-15 -*-
'''
Module Créé le 19 déc. 2012

@author: Python4D/damien compilation de floatctrl.py et events.py du projet zoobie (cf note)
@note: http://nullege.com/codes/show/src@t@h@thezombieengine-HEAD@zombie@code@conjurer@conjurer@gui@floatctrl.py
'''
...
class FloatCtrl(_FloatCtrl):
    ...
    def __on_change_float(self, event):
        self.GetEventHandler().ProcessEvent(
            ChangingEvent( self.GetId(), self.get_value(), self )
            )
        event.Skip()
    ...
    def __on_kill_focus(self, event):
        if self.get_value() != self.old_value:
            self.GetEventHandler().ProcessEvent(
                ChangedEvent( self.GetId(), self.get_value(),
                    self.old_value, self )
                )
            self.old_value = self.get_value()
        event.Skip()
